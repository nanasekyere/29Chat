...
export async function getRoomMessagesCache(roomId: string, limit: number, offset: number): Promise<Message[] | null> {
  const data = await redis.get(`room:${roomId}:messages:${limit}:${offset}`);
  return data ? JSON.parse(data) : null
};

export async function setRoomMessagesCache(roomId: string, limit: number, offset: number, messages: Message[]) {
  await redis.set(`room:${roomId}:messages:${limit}:${offset}`, JSON.stringify(messages), 'EX', 300);
};
